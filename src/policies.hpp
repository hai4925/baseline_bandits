...
public:
  ...
  virtual int sample_arm(boost::random::mt19937& rng) const {
    double x = boost::random::uniform_real_distribution<>(0, norm_const())(rng);
    double c = 0;
  ...
  virtual double get_prob(int arm) const {
    return unnorm_prob(arm) / norm_const();
  }

  virtual Eigen::VectorXd get_grad(int arm) const {
